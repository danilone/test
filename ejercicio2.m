...
error = 10;
count = 0;
while(error > 0.00001 && count<100000)
    listRandonPos = randperm(nPatrones);
for i = listRandonPos%1: nPatrones
    entrada = Patrones(i,:);
    %CalculateSSalidaEjercicio2(w1(:,1),p);
    v1 = CalculateSSalidaEjercicio2(w1(1,:),entrada);
    v2 = CalculateSSalidaEjercicio2(w1(2,:),entrada); 
   ...
   %if sFinal ~= salidaDeseada
        ...
        h1 = dot(w1(1,:), entrada);
        h2 = dot(w1(2,:), entrada);
        deltaCapaIntermedia = calculateDeltaPrecedingLayer(w2,deltaCapaFinal,0, [h1 h2]);
